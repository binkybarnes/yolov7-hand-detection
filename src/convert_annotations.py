...
def convert_to_yolo(info_dict, images_path):
    print_buffer = []
    ...
    for b in info_dict["bboxes"]:
        ...
        # Transform the bbox coordinates as per the format required by YOLOv7
    ...
    save_file_name = os.path.join(
        images_path, info_dict["filename"].replace("jpg", "txt")
    )
...
def main():
    ...
    parser.add_argument(
        "--annotations", type=str, required=True, help="Path to annotations txt"
    )
    parser.add_argument(
        "--plot", action="store_true", help="To plot converted bboxes on a random image"
    )
    ...
    if opt.plot:
        ...
        with open(
            opt.images + "/" + annotation_file.replace("jpg", "txt"), "r"
        ) as file:
